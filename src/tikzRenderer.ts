...
export class TikzRenderer {
  ...
  constructor(plugin: OpencodePlugin) {
    this.plugin = plugin;
    this.cacheDir = join(tmpdir(), "opencode-vault-tikz");
  ...
  private async doRender(tex: string, dir: string, svgFile: string, s: OpencodeSettings): Promise<string> {
...
export function tikzFromSettings(s: OpencodeSettings): string {
  return s.tikzExtraPreamble;
}